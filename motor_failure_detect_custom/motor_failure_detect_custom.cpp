...
MotorFailureDetect::checkRYPError()
{
	...
	// Check if errors exceed thresholds for roll, pitch (code here)
	...
	if (_failed_motor_index == -1) {
        if (roll_fail_flag || pitch_fail_flag) {
            // Motor failure detection cases (code here)
...
void MotorFailureDetect::check_esc_failure()
{
    ...
    if (dt > SUDDEN_CHANGE_THRESHOLD) {
        // Update current for each ESC (code here)

        // Update the current history (code here)

        // Update the last current check time
        _last_current_check_time = now;
    }

    // Update the average current matrix and compute the overall average (code here)

    // Check if the moving average current is below the threshold (code here)
    if () {
        // Check for ESC failure based on variance (code here)
        for () {
    ...
    // Calculate the variance matrix for further analysis (code here)
...
{
    // Step 1: Calculate the average of the current readings (code here)

    // Step 2: Shift elements in _avg_current_mat to the left (code here)

    // Insert the new average current at the end (code here)
}


float
MotorFailureDetect::computeOverallAverage(const matrix::Vector<float, 10>& avg_current_mat) // (code here)
{
...
{
    // Shift elements in _current_history left for each motor (code here)
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 9; ++j) {

        }

        // Add the latest current reading to the end of the history for each motor (code here)
    }
}
...
{
    for (int i = 0; i < 4; ++i) { // (code here)
        float mean = 0.0f;

        // Calculate mean for the current motor (code here)

        // Calculate variance for the current motor (code here)
    }
}
void
